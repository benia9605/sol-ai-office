...
| 파일 | 역할 |
|------|------|
| `src/utils/actionExtractor.ts` | 정규식 기반 액션 추출 (회의 액션/체크리스트/~하기 패턴) |
...
| `src/components/MessageBubble.tsx` | 액션 칩 UI + `onSaveAction` 콜백 |
| `src/components/ChatModal.tsx` | 칩 클릭 → SaveModal 연결 (prefill) |
