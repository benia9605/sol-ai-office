...
1인 사업에서 가장 부족한 건 **다양한 관점**이다. 전략/마케팅/기술/리서치 4개 시각으로 동시에 의견을 받으면 혼자서는 놓치는 맹점을 잡을 수 있다. 현재 회의실은 모디 1명만 답변하는 일반 채팅방이므로, 원래 기획대로 멀티 AI 순차 토론을 구현한다.

### 현재 상태

| 항목 | 상태 |
|------|------|
| 회의실 방 설정 (`data.ts`, `aiModel: 'All'`) | ✅ 있음 |
| 회의실 시스템 프롬프트 (`/prompts/meeting.md`) | ✅ 있음 (순차 구조 텍스트만) |
| 모디가 전체 방 요약 context 수신 (`context.ts`) | ✅ 있음 |
| 멀티 AI 순차 API 호출 로직 | ❌ 없음 |
| 각 AI별 메시지 버블 구분 UI | ❌ 없음 |
| 회의록 자동 생성 | ❌ 없음 |
...
```
유저 메시지 입력
  → ① 모디: "좋은 주제네요! 각 팀원에게 의견을 물어볼게요." (회의 시작 멘트)
  → ② 플래니(전략): 전략적 관점 의견
  → ③ 마키(마케팅): 마케팅/고객 관점 의견
  → ④ 데비(기술): 기술적 실현 가능성 의견
  → ⑤ 서치(리서치): 시장 데이터/경쟁사 기반 의견
  → ⑥ 모디(정리): 전체 의견 종합 + 결정사항 + 액션아이템
```

#### 1-2. DB 스키마 (새 테이블)

```sql
-- 회의록 테이블 (기존 CLAUDE.md에 정의되어 있으나 미생성)
CREATE TABLE meeting_notes (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  conversation_id UUID REFERENCES conversations(id) ON DELETE CASCADE,
  user_id UUID NOT NULL,
  summary TEXT,
  action_items JSONB,      -- [{ task: "", assignee: "플래니", deadline: "" }]
  key_decisions JSONB,     -- [{ decision: "", reason: "" }]
...
**`src/hooks/useChat.ts`** — 핵심 변경

```typescript
// 새 함수: sendMeetingMessage()
// 기존 sendMessage()와 분리하여 회의 전용 로직 추가
...
async function sendMeetingMessage(content: string) {
  ...
  // 2. 모디 시작 멘트 (짧게)
  const modiOpener = await sendChatMessage(modiOpeningPrompt, [...], 'meeting');
  await addMessage(convId, 'assistant', modiOpener, '모디');

  // 3. 4명 AI 순차 호출 (이전 AI 답변을 context에 포함)
  const meetingContext = [{ role: 'user', content }];

  for (const participant of MEETING_ORDER) {
    // participant: { roomId: 'strategy', name: '플래니', focus: '전략적 관점' }
    const prompt = buildMeetingParticipantPrompt(participant, meetingContext);
    const response = await sendChatMessage(prompt, meetingContext, participant.roomId);

    await addMessage(convId, 'assistant', response, participant.name);
    meetingContext.push({ role: 'assistant', content: `[${participant.name}] ${response}` });

    // UI에 실시간 추가 (각 AI 답변이 하나씩 나타남)
    setMessages(prev => [...prev, { sender: 'ai', aiName: participant.name, content: response }]);
  }

  // 4. 모디 정리 (종합 + 액션아이템)
  const modiSummary = await sendChatMessage(modiClosingPrompt, meetingContext, 'meeting');
  await addMessage(convId, 'assistant', modiSummary, '모디');

  // 5. 회의록 자동 저장
  await saveMeetingNote(convId, modiSummary);
}

const MEETING_ORDER = [
  { roomId: 'strategy',  name: '플래니', focus: '전략/의사결정 관점' },
  { roomId: 'marketing', name: '마키',   focus: '마케팅/고객 관점' },
  { roomId: 'dev',       name: '데비',   focus: '기술적 실현가능성' },
  { roomId: 'research',  name: '서치',   focus: '시장 데이터/경쟁사' },
];
```

**`src/components/ChatModal.tsx`** — 회의 UI

```
변경 사항:
- room.id === 'meeting'일 때 메시지 버블에 AI 아바타+이름 항상 표시
- "회의 진행 중..." 로딩 인디케이터 (현재 발언 중인 AI 표시)
- 회의 종료 후 "회의록 보기" 버튼 표시
```

**`src/components/MessageBubble.tsx`** — AI 구분 표시

```
변경 사항:
- aiName prop으로 아바타 이미지 + 이름 렌더링
- 회의실에서는 각 AI별 accent color 적용
  - 플래니: 보라 / 마키: 핑크 / 데비: 라임 / 서치: 브라운 / 모디: 노랑
```

**`src/services/chatApi.ts`** — 회의 참가자별 API 호출

```
변경 사항:
- buildMeetingParticipantPrompt(participant, previousResponses) 함수 추가
- 각 AI의 base 프롬프트 + "회의 토론 중이며, 아래는 이전 발언" context 주입
```

**`src/services/meetingNotes.service.ts`** — 신규 파일

```
기능:
- saveMeetingNote(conversationId, modiSummary)
  → modiSummary에서 액션아이템/결정사항 파싱
  → meeting_notes 테이블에 저장
- fetchMeetingNotes(conversationId)
- parseMeetingActions(text) → { task, assignee, deadline }[]
```

**`public/prompts/meeting.md`** — 프롬프트 업데이트

```
추가할 내용:
- 모디 시작 멘트 가이드 (1-2문장으로 짧게)
- 각 AI별 회의 참가 프롬프트 템플릿
- 모디 정리 포맷:
  ## 회의 정리
  ### 핵심 결정사항
  - ...
...
- API 호출이 5회(모디 시작 + 4명 + 모디 정리 = 6회)로 비용이 높음
- 각 AI 응답 길이를 `maxTokens: 500`으로 제한 권장 (모디 정리만 1000)
- 유저가 중간에 추가 발언할 수 있는 옵션은 v2에서 고려
...
| 항목 | 상태 |
|------|------|
| SaveModal (수동 저장: 할일/일정/인사이트/기록/스터디) | ✅ 있음 |
| MessageBubble에 ⋯ 메뉴 → SaveModal 연결 | ✅ 있음 |
| AI 응답 자동 파싱 | ❌ 없음 |
| 자동 추출 UI (인라인 제안) | ❌ 없음 |
...
AI 응답 수신
  ...
  → 감지된 항목이 있으면 메시지 하단에 제안 칩 표시
  → 유저가 칩 클릭 → 해당 항목 pre-fill된 SaveModal 열림
  → 저장 or 무시
```

#### 2-2. 추출 로직 (2단계)

**Phase 1: 정규식 기반 (빠르고 무료)**
...
interface ExtractedAction {
  ...
  title: string;
  date?: string;        // 감지된 날짜
  priority?: string;    // "긴급", "중요" 등 키워드 감지
  originalText: string; // 원문
}

function extractActionItems(text: string): ExtractedAction[] {
  ...
  const taskPatterns = [
    ...
    /(?:→|▶|►)\s*(.+(?:하기|필요|해야|하세요|해보세요|추천))/gm,
    /\*\*(.+?)\*\*.*(?:하세요|해보세요|필요|추천)/gm,
  ];

  // 패턴 2: 날짜 감지
  // "3월 5일", "이번 주", "다음 주 월요일", "3/5", "내일까지"
  const datePattern = /(\d{1,2}월\s*\d{1,2}일|\d{1,2}\/\d{1,2}|이번\s*주|다음\s*주|내일|모레|오늘)/g;

  // 패턴 3: 인사이트/핵심 포인트
  // "핵심:", "포인트:", "💡", "중요:"
  const insightPatterns = [
    /(?:핵심|포인트|인사이트|중요|팁)[:\s]*(.+)/gm,
    /💡\s*(.+)/gm,
  ];

  // ... 패턴 매칭 후 ExtractedAction[] 반환
  return actions;
}
```

**Phase 2: AI 기반 추출 (선택적, 더 정확)**

```typescript
// 시스템 프롬프트에 추가하는 방식
// AI가 응답 마지막에 JSON 블록으로 액션 아이템을 함께 반환

// 시스템 프롬프트 추가:
`
응답 마지막에 아래 형식으로 액션 아이템을 추출해주세요 (없으면 생략):
<!--actions
[{"type":"task","title":"사주궁합 프롬프트 마무리","date":"2026-03-05"}]
actions-->
`

// 파싱:
function parseAIActions(response: string): { cleanText: string; actions: ExtractedAction[] } {
  const match = response.match(/<!--actions\n([\s\S]*?)\nactions-->/);
  if (!match) return { cleanText: response, actions: [] };

  const actions = JSON.parse(match[1]);
  const cleanText = response.replace(/<!--actions[\s\S]*?actions-->/, '').trim();
...
**`src/components/MessageBubble.tsx`** — 액션 제안 칩
...
칩 클릭 → SaveModal 열림 (type + title pre-fill)
```
...
| 파일 | 변경 내용 |
|------|----------|
| `src/utils/actionExtractor.ts` | **신규** — 추출 로직 |
| `src/components/MessageBubble.tsx` | 액션 칩 UI 추가 |
| `src/components/ChatModal.tsx` | 칩 클릭 → SaveModal 연결 |
| `src/hooks/useChat.ts` | AI 응답 수신 후 extractActionItems() 호출 |

#### 2-5. 의존성

- 없음 (독립적으로 구현 가능)
- Phase 2(AI 기반 추출)는 시스템 프롬프트 수정 필요 → `public/prompts/*.md` 전체
...
CREATE TABLE daily_briefings (
  ...
  --   urgentTasks: [{ title, dDay }],
  --   projectProgress: [{ name, percent, note }],
  --   aiComment: "오늘의 한마디 텍스트"
  -- }
  created_at TIMESTAMPTZ DEFAULT NOW(),
  UNIQUE(user_id, date)  -- 하루에 한 번만
);
```
...
async function generateDailyBriefing(): Promise<BriefingData> {
  const userId = await getCurrentUserId();
  const today = new Date().toISOString().split('T')[0];

  // 1. 이미 오늘 브리핑이 있는지 확인
  const existing = await fetchBriefing(userId, today);
  if (existing) return existing;

  // 2. 데이터 수집 (Promise.all로 병렬)
  const [schedules, tasks, goals, yesterdaySummaries] = await Promise.all([
    fetchTodaySchedules(userId),
    fetchUrgentTasks(userId),
    fetchActiveGoals(userId),
    fetchYesterdaySummaries(userId),
  ]);

  // 3. 모디에게 "오늘의 한마디" 생성 요청
  const aiComment = await generateModiComment({
    schedules,
    tasks,
    goals,
    yesterdaySummaries,
  ...
  const briefingData: BriefingData = {
    schedules: schedules.slice(0, 5).map(s => ({ title: s.title, time: s.time })),
    urgentTasks: tasks.slice(0, 5).map(t => ({ title: t.title, dDay: calcDDay(t.deadline) })),
    projectProgress: goals.map(g => ({ name: g.project, percent: g.progress, note: g.title })),
    aiComment,
  };

  // 5. DB에 캐싱
  await saveBriefing(userId, today, briefingData);

  return briefingData;
}

async function generateModiComment(data: BriefingInput): Promise<string> {
  // Claude API 호출 (secretary 모델 사용)
  const prompt = `
당신은 모디입니다. 아래 데이터를 보고 Sol님에게 오늘 하루를 위한
격려+우선순위 제안을 1-2문장으로 해주세요. 따뜻하고 실용적으로.

오늘 일정: ${JSON.stringify(data.schedules)}
긴급 할일: ${JSON.stringify(data.tasks)}
목표 진행률: ${JSON.stringify(data.goals)}
어제 대화 요약: ${JSON.stringify(data.yesterdaySummaries)}
`;

  return await sendChatMessage(prompt, [], 'secretary', 200); // maxTokens: 200
}
```
...
```
옵션 A (권장): 홈페이지 진입 시 lazy 생성
  - DashboardWidgets 로드 시 generateDailyBriefing() 호출
  - 이미 오늘 브리핑 있으면 DB에서 바로 반환 (API 호출 없음)
  - 없으면 생성 후 반환

옵션 B: 새벽 자동 생성 (Supabase Edge Function / Cron)
  - 매일 06:00 자동 실행
  - 앱 열면 이미 준비된 브리핑 표시
  - 상용화 시 적합하지만 서버 비용 발생
```
...
| 파일 | 변경 내용 |
|------|----------|
| `src/services/briefing.service.ts` | **신규** — 브리핑 생성/조회/저장 |
| `src/hooks/useBriefing.ts` | **신규** — 브리핑 데이터 로드 훅 |
| `src/components/DashboardWidgets.tsx` | 브리핑 카드 컴포넌트 추가 (기존 위젯 위) |
| `src/components/BriefingCard.tsx` | **신규** — 브리핑 UI 컴포넌트 |
| `public/prompts/modi.md` | 브리핑용 프롬프트 가이드 추가 |
