...
| AI 응답 정규식 파싱 (`actionExtractor.ts`) | ✅ 완료 |
| 인라인 제안 칩 UI (MessageBubble 하단) | ✅ 완료 |
| 칩 클릭 → SaveModal 제목/날짜/우선순위 프리필 | ✅ 완료 |
| 일반 1:1 채팅 + 회의실 양쪽 지원 | ✅ 완료 |
| 명시적 할일 섹션만 추출 (`hasActionSection` 게이트) | ✅ 완료 |
| "전체 추가" 버튼 (모달 없이 일괄 등록) | ✅ 완료 |
| 할일 페이지 멀티 선택 모드 (리스트뷰 + 칸반뷰) | ✅ 완료 |
| 일괄 편집 패널 (목표/중요도/날짜/종류/상태) | ✅ 완료 |

### 구현된 파일

| 파일 | 역할 |
|------|------|
| `src/utils/actionExtractor.ts` | 정규식 기반 액션 추출 (회의 액션/체크리스트/~하기 패턴) + `hasActionSection` 게이트 |
| `src/types.ts` | ChatMessage에 `extractedActions` 필드 + SaveModalConfig에 prefill 필드 |
| `src/hooks/useChat.ts` | AI 응답 후 `extractActionItems()` 호출 |
| `src/components/MessageBubble.tsx` | 액션 칩 UI + 개별 `onSaveAction` + "전체 추가" `onBulkSaveActions` 콜백 |
| `src/components/ChatModal.tsx` | 칩 클릭 → SaveModal 연결 (prefill) + `handleBulkSaveActions` (모달 없이 일괄 등록) |
| `src/components/SaveModal.tsx` | `prefilledTitle/Date/Priority` prop 지원 |
| `src/pages/TasksPage.tsx` | 멀티 선택 모드 (`selectMode` + `selectedIds`) + 일괄 편집 패널 |
| `src/components/tasks/TaskListItem.tsx` | 선택 모드 시 체크박스 표시 (StatusIcon 대체) |
| `src/components/tasks/TaskListView.tsx` | selectMode/selectedIds/onToggleSelect props 전달 |
| `src/components/tasks/TaskKanbanView.tsx` | 칸반 카드에 선택 모드 체크박스 + 하이라이트 |
...
```
AI 응답에서 액션아이템이 2개 이상 추출되면:
  → 개별 칩 아래 "전체 추가 (N개)" 버튼 표시 (bg-green-500)
  → 클릭 시 모달 없이 즉시 addTask() loop로 일괄 등록
  → 대화 컨텍스트 1회만 저장 (conversation_id 공유)
  → 완료 시 "N개 할일이 추가되었습니다" 토스트
```

#### 2-7. 할일 멀티 선택 + 일괄 편집

```
TasksPage 상단에 "선택" 토글 버튼:
  → 선택 모드 ON: StatusIcon 대신 체크박스 표시
  → 선택된 항목: 초록색 링 하이라이트
  → 상단 툴바: "N개 선택됨 | 전체 선택 | 일괄 편집"
  → 일괄 편집 패널 (드롭다운):
    - 목표: GoalSelect 컴포넌트 재사용
    - 중요도: high/medium/low 칩
    - 마감일: date input
    - 종류: 기존 카테고리 칩
    - 상태: pending/in_progress/completed 칩
  → "적용" → 선택된 필드만 updateTask() 일괄 호출
  → 리스트뷰 + 칸반뷰 모두 지원
```
...
| 브리핑 카드 UI (접기/펼치기, 새로고침) | ✅ 완료 |
| 오늘 일정 / 긴급 할일 / 프로젝트 진행률 표시 | ✅ 완료 |
| 프로젝트 진행률 PC 3열 그리드 | ✅ 완료 |
| 모디 AI 한마디 생성 (Claude secretary, 200토큰) | ✅ 완료 |
| AI 한마디 daily_briefings 테이블 캐싱 (하루 1회) | ✅ 완료 |
| [모디에게 물어보기] → 비서방 채팅 연결 | ✅ 완료 |

### 구현된 파일

| 파일 | 역할 |
|------|------|
| `src/services/briefing.service.ts` | 데이터 수집 + AI 한마디 생성/캐싱 |
| `src/hooks/useBriefing.ts` | 브리핑 데이터 로드 훅 |
| `src/components/BriefingCard.tsx` | 브리핑 카드 UI (접기/펼치기) |
| `src/pages/HomePage.tsx` | BriefingCard 배치 + 모디 연결 |
...
## 6. AI 장기 기억 (Context Memory) — 1단계 완료
...
앱의 핵심 포지션: **긴 대화는 Claude 앱에서, AI 오피스는 "내 현상황 파악한 비서들"**
→ 장기 대화보다 **컨텍스트(현황 데이터)의 질**이 더 중요
→ 같은 대화 안에서는 티키타카 가능 + 대화 넘어가면 요약으로 맥락 유지 + 비용 최소화
...
| 같은 대화 세션 내 최근 20개 메시지 전문 전송 | ✅ 있음 (슬라이딩 윈도우) |
| 1:1 방 "새 대화" 버튼 + 자동 요약 저장 | ✅ 완료 |
| 비서방 "오늘 요약" (수동, 전체 방 요약) | ✅ 있음 |
| 프로젝트별 핵심 메모/결정사항 (📌 기억해) | ❌ Phase 2 |
| 유저가 직접 "이거 기억해" 명령 | ❌ Phase 2 |

### 1단계 구현 완료: "새 대화" + 자동 요약

#### 6-0. 구현된 파일

| 파일 | 역할 |
|------|------|
| `src/services/summary.service.ts` | `summarizeConversation()` — 대화 메시지를 AI 요약 → DB 저장 |
| `src/hooks/useChat.ts` | `startNewChat()` — 현재 대화 자동 요약 → 구분선 → conversation 리셋 |
| `src/components/ChatModal.tsx` | 1:1 방 헤더에 "+ 새 대화" 버튼 + 보라색 구분선 |

#### 6-0a. 대화 맥락 유지 구조

```
[단기 기억] 같은 대화 세션 내
  → 최근 20개 메시지 전문 전송 (useChat.ts:239 .slice(-20))
  → 자연스러운 티키타카 가능

[중기 기억] "새 대화" 전환 시
  → 현재 대화를 AI가 자동 요약 → conversation_summaries 테이블 저장
  → 유저는 안 봐도 됨 (백그라운드)
  → 다음 대화 시 시스템 프롬프트에 요약 자동 주입 (context.ts)

[상시 컨텍스트] 매 대화마다 시스템 프롬프트에 포함
  → 유저 정보 (이름/소개/대화 스타일)
  → 프로젝트 & 목표 & KPI (진행률)
  → 7일 이내 일정
  → 진행 중 할일 (우선순위, 마감일)
  → 최근 대화 요약 (해당 방 + 모디는 전체 방)
```

#### 6-0b. "새 대화" 흐름

```
1:1 채팅방 (플래니/마키/데비/서치/모디)
  → 대화 진행 (티키타카)
  → [+ 새 대화] 클릭
  → 백그라운드: summarizeConversation() → conversation_summaries DB 저장
  → ✦ 새 대화 ✦ 보라색 구분선 표시
  → conversationId 리셋 → 다음 메시지부터 새 conversation
  → AI가 시스템프롬프트에서 이전 요약 참고

회의실은 기존 "새 회의 시작" 버튼 사용 (💛 노란색 구분선)
비서방 "오늘 요약"은 유저가 직접 보는 용도 (별도 유지)
```
