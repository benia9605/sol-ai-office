...
**작성 원칙:**
- **파일 내용 첫 줄에도 번호를 넣는다** (파일명뿐 아니라 SQL 상단 주석에 `-- NNN: 제목`). 대시보드 SQL 에디터에 붙여넣고 볼 때 어느 마이그레이션인지 헷갈리지 않게. 이어서 **배경(왜 만드는지)·무엇을 하는지**를 주석으로 정리. 예:
  ```sql
  -- ────────────────────────────────────────────────────────
  -- 049: 제품별 재고 집계 뷰
  -- ────────────────────────────────────────────────────────
  -- 배경: 재고 현황 페이지가 재고를 조회하지 않고 unlimited_stock 여부만 찍고 있었다.
  -- 이 뷰는 제품별 입고/출고/조정/현재재고를 한 번에 계산한다. security_invoker=true 로 호출자 RLS 적용.
  ```
